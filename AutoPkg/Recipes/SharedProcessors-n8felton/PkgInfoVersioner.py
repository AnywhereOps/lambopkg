# Downloaded from https://github.com/autopkg/n8felton-recipes/blob/646d469b0e6e964e0605774d8541ce29912b9207/SharedProcessors/PkgInfoVersioner.py
...
class PkgInfoVersioner(Processor):
    ...
    input_variables = {
        "unpacked_path": {
            "required": True,
            "description": (
                "The path where the package containing the "
    ...
    def main(self):
        ...
        pkg_ref_xpath = './/pkg-ref[@id="{}"][@version]'.format(self.env["PKGID"])
        pkg = getattr(distribution.find(pkg_ref_xpath), "text", None)
        if pkg is None:
            raise ProcessorError("PKGID was not found.")
        pkg_path = pkg.lstrip("#")
        self.output(f"Package file: {pkg_path}", 3)
        packageinfo_path = "{}/{}/{}".format(self.env["unpacked_path"], pkg_path, "PackageInfo")
        self.output(f"PackageInfo file: {packageinfo_path}", 3)
