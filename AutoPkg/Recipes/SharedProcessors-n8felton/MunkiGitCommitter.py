# Downloaded from https://github.com/autopkg/n8felton-recipes/blob/646d469b0e6e964e0605774d8541ce29912b9207/SharedProcessors/MunkiGitCommitter.py
...
from autopkglib import Processor, get_pref

__all__ = ["MunkiGitCommitter"]


class MunkiGitCommitter(Processor):
    description = "Allows AutoPkg to commit changes to a munki repository that is tracked by a git repository."
    input_variables = {
        "MUNKI_REPO": {"description": "Path to a mounted Munki repo.", "required": True},
        "GIT_COMMIT_MESSAGE": {
            "required": False,
            "description": ("Any additional message you want attached to the commit."),
        },
        "munki_importer_summary_result": {"required": False, "description": "Stuff goes here"},
    }
    output_variables = {}

    def git_cmd(self):
        "Returns a path to a git binary, priority in the order below. Returns None if none found.1. app pref 'GIT_PATH'2. a 'git' binary that can be found in the PATH environment variable3. '/usr/bin/git'"
        ...
        if git_path_pref:
            if is_executable(git_path_pref):
                ...
                log_err(
                    "WARNING: Git path given in the 'GIT_PATH' preference:"
                    " '%s' either doesn't exist or is not executable! "
                    "Falling back to one set in PATH, or /usr/bin/git." % git_path_pref
                )
    ...
    def run_git(self, git_options_and_arguments, git_directory=None):
        ...
        try:
            proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=git_directory)
            (cmd_out, cmd_err) = proc.communicate()
        except OSError as err:
            raise GitError("ERROR: git execution failed with error code %d: %s" % (err.errno, err.strerror))
    ...
    def main(self):
        ...
        pkginfo_path = "{}/{}".format("pkgsinfo", self.env["munki_importer_summary_result"]["data"]["pkginfo_path"])
        ...
        if self.env.get("GIT_COMMIT_MESSAGE"):
            ...
            commit_message = f"[AutoPkg] Adding {name} version {version}"

        self.run_git(["add", pkginfo_path], git_directory=self.env["MUNKI_REPO"])
        self.run_git(["commit", "-m", commit_message], git_directory=self.env["MUNKI_REPO"])
