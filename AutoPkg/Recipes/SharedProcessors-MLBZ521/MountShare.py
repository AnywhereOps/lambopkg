# Downloaded from https://github.com/autopkg/MLBZ521-recipes/blob/8b7738eed4c2f8b6b32a897907ee3420fa0c74ef/Shared Processors/MountShare.py
...
class MountShare(Processor):
    ...
    description = __doc__
    input_variables = {
        "url": {"description": "An SMB URL to mount.", "required": True},
        "share_name": {"description": "Share to mount from the SMB server.", "required": True},
        "port": {
        ...
        "protocol": {
            "description": (
                "The protocol used to connect to the url.  The default and currently only supported protocol is 'smb'."
            ),
            "default": "smb",
            "required": False,
        },
        "username": {"description": "Username, if required, to connect to the SMB server.", "required": False},
        "password": {"description": "Password, if required, to connect to the SMB server.", "required": False},
        "domain": {"description": "Domain, if required, to connect to the SMB server.", "required": False},
    }
    output_variables = {}
    required_attrs = {
        "url",
    ...
    def __repr__(self):
        ...
        output = [f"Share:  {self.connection['url']}", f"Type: {type(self)}", "Connection Information:"]
    ...
    def _build_url(self):
        ...
        self.connection["mount_url_passwordless"] = (
            f"//{pwless}{self.connection['url']}{port}/{self.connection['share_name']}"
        )
