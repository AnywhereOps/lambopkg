# Downloaded from https://github.com/autopkg/MLBZ521-recipes/blob/8b7738eed4c2f8b6b32a897907ee3420fa0c74ef/Shared Processors/OfflineApps.py
...
class OfflineApps(URLDownloader):
    ...
    description = __doc__
    input_variables = {
        "search_path": {"description": "Root path to search within.", "required": True},
        "OFFLINEAPPS_URL": {
            "description": ("An optional SMB URL to mount containing the search path."),
            "required": False,
        },
        "OFFLINEAPPS_SHARE_NAME": {"description": "Share to mount from the SMB server.", "required": False},
        "OFFLINEAPPS_PORT": {
            "description": ("Port to use to connect to the SMB server.  Default port used is 445."),
            "default": "445",
            "required": False,
        },
        "OFFLINEAPPS_MOUNT_POINT": {
            "description": (
                "Where the SMB share will be mounted too.  Default mount point will be '/private/tmp/OfflineApps'."
            ),
            "default": "/private/tmp/OfflineApps",
            "required": False,
        },
        "OFFLINEAPPS_USERNAME": {"description": "Username required to connect to the SMB server.", "required": False},
        "OFFLINEAPPS_PASSWORD": {"description": "Password required to connect to the SMB server.", "required": False},
        "OFFLINEAPPS_DOMAIN": {"description": "Domain, if required, to connect to the SMB server.", "required": False},
        "OFFLINEAPPS_PROTOCOL": {
            "description": (
                "The protocol used to connect to the url.The default and currently only supported protocol is 'smbfs'."
            ),
            "default": "smbfs",
            "required": False,
        },
        ...
        "unmount": {
            "description": "Unmount the share when complete.  Default is 'False'",
            "default": False,
            "required": False,
        },
    }
    output_variables = {
        "version": {"description": ("The highest version found according to pkg_resources.parse_version logic.")},
        "found_major_version": {"description": 'The "major version" of the version string found.'},
        "cached_path": {"description": "Path to the existing contents in the AutoPkg Cache."},
        "download_changed": {
    ...
    def download_local_file(self, file_to_download, save_path):
        ...
        if (
            ...
            and self.check_filesize
            and os.path.getsize(file_to_download) == os.path.getsize(destination_path)
        ):
    ...
    def main(self):
        ...
        try:
            ...
            for root_directory, folders, files in self.walk(search_path, int(max_depth)):
                ...
                list_of_versions.extend(
                    os.path.join(root_directory, match)
                    for match in self.check_for_matching(
                        items_to_check,
            ...
            version_folder = " ".join(
                filter(None, [contains_search_string, must_contain_limitation_string, self.env["version"]])
            )
            ...
            if os.path.isdir(version_location):
                ...
                for root_directory, folders, files in self.walk(version_location, 0):
                    for file in files:
                        # Download each file
                        self.download_local_file(f"{version_location}/{file}", self.env["cached_path"])
            ...
            # Optionally unmount
            if unmount and self.env.get("OFFLINEAPPS_URL"):
                self.share.unmount()
