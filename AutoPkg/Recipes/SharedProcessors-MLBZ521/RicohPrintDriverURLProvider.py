# Downloaded from https://github.com/autopkg/MLBZ521-recipes/blob/8b7738eed4c2f8b6b32a897907ee3420fa0c74ef/Shared Processors/RicohPrintDriverURLProvider.py
...
class RicohPrintDriverURLProvider(URLGetter):
    ...
    input_variables = {
        "model": {"required": True, "description": ("The official model name of the Ricoh Printer to search for.")},
        "os_version": {"required": False, "description": "The OS version to search against.", "default": "latest"},
        "web_driver": {
            "required": False,
            "description": (
    ...
    def main(self):
        ...
        curl_opts = [
            "--url",
            "https://www.ricoh-usa.com/api/search",
            "--data-raw",
            json.dumps({
                "requestState": {
                    "current": 1,
                ...
                "queryConfig": {
                    "filters": [{"field": "locale", "values": ["en-US"], "type": "all"}],
                    "result_fields": {"Gen_Linked_Driver_Drivers": {"raw": {}}},
                },
            }),
        ]
        ...
        try:
            drive_page_url = json.loads(result).get("results")[0].get("Gen_Linked_Driver_Drivers").get("raw")
        ...
        with WebEngine(
            ...
            try:
                ...
                WebDriverWait(web_engine, timeout=10).until(lambda d: d.find_element(By.XPATH, section_MacOSX))
            ...
            try:
                ...
                WebDriverWait(web_engine, timeout=10).until(lambda d: d.find_element(By.XPATH, xpath_os_version))
            ...
            try:
                ...
                download_url = section_selected_os_version.find_element(
                    By.XPATH,
                    '//*[@id="os-driver-list"]/div[4]/div/div/div[2]/div[1]/div/div/div[2]/div/div/div/div/div/div[1]/p[2]/a',
                ).get_attribute("href")
            except:
                raise ProcessorError("Failed to find and collect the download url from the download button.")
