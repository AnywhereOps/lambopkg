# Downloaded from https://github.com/autopkg/MLBZ521-recipes/blob/8b7738eed4c2f8b6b32a897907ee3420fa0c74ef/Shared Processors/CanonPrintDriverURLProvider.py
# Commit: 8b7738eed4c2f8b6b32a897907ee3420fa0c74ef
# Downloaded at: 2025-11-27 22:31:12 UTC
...
class CanonPrintDriverURLProvider(URLGetter):
    ...
    description = __doc__
    input_variables = {
        "model": {"required": True, "description": ("The official model name of the Canon Printer to search for.")},
        "os_version": {
            "required": False,
            "description": (
    ...
    output_variables = {"url": {"description": "Returns the url to download."}}
    ...
    def main(self):
        ...
        curl_opts = [
            "--url",
            "https://onewebsearch.usa.canon.com/rest/search/v2?organizationId=canonusaproductionw69lguud",
            "--data-raw",
            f'{{"context":{{"environment":"PROD","website":"CanonProductFinderSearch","softwareDrivers":"softwareDrivers"}},"pipeline":"Canon Product Finder Search","q":"{model}","enableQuerySyntax":false,"searchHub":"CanonProductFinderSearch_support-software-and-drivers-page","numberOfResults":1,"firstResult":0}}',
        ]
        headers = {
        ...
        with WebEngine(
            engine=web_driver,
            binary=web_driver_binary_location,
            path=web_driver_path,
            headless=web_engine_headless,
            parent=self,
        ) as web_engine:
            try:
            ...
            try:
                ...
                self.scroll_into_view_and_click(
                    web_engine, "//*/div[contains(@class, 'accordion-title')][contains(text(), 'Software & Drivers')]"
                )
            ...
            try:
                ...
                web_engine.find_element(
                    By.CSS_SELECTOR,
                    "div[class='software-downloads'] div[class='os-dropdown os-names ada-clickable'] button[class='dropdown-btn']",
                ).click()
                time.sleep(1)
                # Select the "OS type"
                ...
                web_engine.find_element(
                    By.XPATH,
                    "//*/ul[contains(@class, 'dropdown os-names-dropdown')]/li[@class='os-type'][@filtervalue='Mac']",
                ).click()
                time.sleep(1)

                # Open the "OS version" dropdown menu
                os_version_dropdown = web_engine.find_element(
                    By.CSS_SELECTOR,
                ...
                web_engine.find_element(
                    By.XPATH,
                    f"//*/ul[contains(@class, 'dropdown os-versions-dropdown')]/li[@osfamily='Mac'][@filtervalue='{os_version}']/a[@class='os-version__name']",
                ).click()
            ...
            try:
                ...
                web_engine.find_element(
                    By.XPATH, "//*/div[contains(@class, 'os-dropdown medium downloads_sort ada-clickable')]"
                ).click()
                time.sleep(1)
                ...
                if re.match(download_type, "Recommended", re.IGNORECASE):
                    ...
                    # Sort by Recommended
                    web_engine.find_element(
                        By.XPATH, f"//*/ul[@class='dropdown'][@filtertype='sort']/li[@filtervalue='{download_type}']"
                    ).click()
                    ...
                    web_engine.find_element(
                        By.XPATH, "//*/ul[@class='dropdown'][@filtertype='sort']/li[@filtervalue='Date']"
                    ).click()
            ...
            if re.match(download_type, "Recommended", re.IGNORECASE):
                try:
                    download_url = web_engine.find_element(
                        By.XPATH,
                        "//*/div[@class='software-downloads__container']/div[@recommended='Y']//div[@class='download__cta']//a[@role='button']",
                    ).get_attribute("href")
                except:
                    raise ProcessorError("Failed to identify the download url for the Recommended download.")

            else:
                try:
                    # Click the "Load More" button to display all options _if_ the button is
                    # visible...it may be there, but it may not be visible...
                    if load_more_section := web_engine.find_element(
                        By.XPATH, "//*[contains(@class, 'advisories-load-more-button-container')]"
                    ):
                        if load_more := web_engine.find_element(
                            By.XPATH,
                            ...
                            time.sleep(1)
                            web_engine.execute_script(
                                'arguments[0].scrollIntoView({"block": "center", "inline": "center"});', load_more
                            )
                            time.sleep(1)
                            load_more.click()
                            time.sleep(1)
                ...
                download_list = web_engine.find_element(By.XPATH, "//*/div[@class='software-downloads__container']")
                ...
                if download_type in ["UFRII", "PS", "FAX", "PPD"]:
                    try:
                        ...
                        download_version_urls = [
                            ...
                            for link in links
                            if re.match(
                                rf"^{download_type}.+", os.path.basename(link.get_attribute("href")), re.IGNORECASE
                            )
                        ]
                    ...
                    links = download_list.find_elements(
                        By.XPATH, f"//*/a[contains(@class, 'file-name-link')][contains(text(), '{link_text}')]"
                    )
