# Downloaded from https://github.com/autopkg/hansen-m-recipes/blob/de17d7db2537ffa00c27281a392f37fe599efdca/SharedProcessors/CopyImporter.py
...
class CopyImporter(Processor):
    ...
    input_variables = {
        ...
        "overwrite": {
            "required": False,
            "description": (
                "Whether the destination will be overwritten if necessary. Uses a boolean value. Defaults to 'False'."
            ),
        },
    }
    output_variables = {"copyimporter_summary_result": {"description": "Summary of items imported with CopyImporter."}}
    ...
    def copy(self, source_item, dest_item, overwrite=False):
        ...
        try:
            if not os.path.exists(dest_item) or overwrite:
                ...
                self.output(
                    f"Skipping import as 'overwrite'='{overwrite}' and 'destination_path' exists: '{dest_item}'"
                )
    ...
    def main(self):
        ...
        try:
            ...
            if not os.path.exists(source_path) or os.path.isdir(source_path):
                raise ProcessorError(f"source_path '{source_path}' doesn't exist or is a directory")

            # process path with glob.glob
            matches = glob.glob(source_path)
            if len(matches) == 0:
                raise ProcessorError(f"Error processing path '{source_path}' with glob. ")
            matched_source_path = matches[0]
            if len(matches) > 1:
                self.output(f"WARNING: Multiple paths match 'source_path' glob '{source_path}':")
                for match in matches:
                    self.output(f"  - {match}")

            if [c for c in "*?[]!" if c in source_path]:
                self.output(f"Using path '{matched_source_path}' matched from globbed '{source_path}'.")
            ...
            if os.path.isdir(destination_path) or destination_path.endswith("/"):
                destination_path = os.path.join(destination_path, os.path.basename(matched_source_path))
