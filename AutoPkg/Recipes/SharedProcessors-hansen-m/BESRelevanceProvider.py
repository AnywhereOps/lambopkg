# Downloaded from https://github.com/autopkg/hansen-m-recipes/blob/de17d7db2537ffa00c27281a392f37fe599efdca/SharedProcessors/BESRelevanceProvider.py
# Commit: de17d7db2537ffa00c27281a392f37fe599efdca
# Downloaded at: 2025-11-27 20:29:06 UTC
...
class BESRelevanceProvider(Processor):
    ...
    input_variables = {
        "bes_filepath": {
            "required": False,
            "description": "Path to a file for relevance data. Defaults to %pathname%.",
        },
        "bes_relevance": {
            "required": False,
            "description": "A line of relevance to evaluate in QnA and return the result.",
        },
        "output_var_name": {
            "required": False,
            "description": "Output variable name. Defaults to 'bes_relevance_result'",
        },
    }
    output_variables = {
        "bes_sha1": {"description": "The resulting file sha1 of the %bes_filepath%."},
        "bes_sha1_short": {"description": "The short end of %bes_sha1% to be used as a display version."},
        "bes_size": {"description": "The resulting file size of the %bes_filepath%."},
        "bes_sha256": {"description": "The resulting file size of the %bes_filepath%."},
        "output_var_name": {"description": "Output variable, defaults to 'version'"},
    }
    __doc__ = description
    ...
    def eval_relevance(self, relevance):
        ...
        try:
            proc = subprocess.Popen(
                QNA, bufsize=-1, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            if output.get("E"):
                self.output("Relevance Error: {%s} -- %s" % (relevance, output.get("E")))
                return None
            else:
                return output.get("A")
    ...
    def main(self):
        ...
        if bes_relevance:
            output_var_name = self.env.get("output_var_name", "bes_relevance_result")
            ...
            self.output("%s = %s" % (output_var_name, self.env.get(output_var_name)))
